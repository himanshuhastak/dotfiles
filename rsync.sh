...
SSH_CIPHER="aes128-gcm@openssh.com"     ## Should be available on all new machines

time  eval rsync $RSYNC_ARGS --skip-compress=$RSYNC_SKIP_COMPRESS  -e \"ssh -T -c $SSH_CIPHER -o Compression=no -x \" $* ||  EXIT_STATUS=$?

## if failure due to unknown cipher; then try older arcfour
