# lib/common.sh — shared helpers for dotfiles install scripts.
# Source from any script:  source "$(dirname "$0")/../lib/common.sh"
# (tool scripts live one level deeper:  ../../lib/common.sh)

[ -n "${DOTFILES_COMMON_SOURCED:-}" ] && return 0
DOTFILES_COMMON_SOURCED=1

# --- paths (everything self-contained under the repo) ------------------------
DOTFILES="$(cd "${DOTFILES:-$(dirname "${BASH_SOURCE[0]}")/..}" && pwd)"
STOW_DIR="$DOTFILES/stow"
STOW="$DOTFILES/bin/stow"
TOOLS_DIR="$DOTFILES/.tools"        # self-owned; NEVER reuse system/NFS tools
BIN="$TOOLS_DIR/bin"
