...
ver="$(/usr/bin/curl -fsSL --connect-timeout 20 "https://ftp.gnu.org/gnu/bash/" 2>/dev/null \
  | /usr/bin/grep -oE 'bash-[0-9]+\.[0-9]+\.[0-9]+\.tar\.gz' \
  | /usr/bin/sed 's/\.tar\.gz//' | sort -V | tail -n1 | /usr/bin/sed 's/^bash-//')"
...
if /usr/bin/curl -fL --retry 3 --connect-timeout 20 -o "$tmp/bash.tar.gz" "$url" \
