...
resolve_zsh_url() {
  ...
  for v in 5.9 5.8 5.7; do
    for url in \
      ...
      if /usr/bin/curl -fsSLI --connect-timeout 15 "$url" 2>/dev/null \
         | /usr/bin/head -n1 | /usr/bin/grep -q '200\|302'; then
        printf '%s %s\n' "$v" "$url"
        return 0
      fi
    done
...
if /usr/bin/curl -fL --retry 3 --connect-timeout 30 -o "$tmp/zsh.tar.xz" "$url" \
